...
<#
  ...
  version 1.0.0.8 skipped. internal testing version
  version 1.0.0.9
    -  Changed the Fluxor Write-* functions to use an array of objects when writing to InfluxDB (resolves Issue #13).
    -  Added Repaint parameter to return the last hour of performance stats (compared to the default of realtime).
       When using Repaint, some results for device, iops, etc. will not be returned, depending on your vCenter configuration for performance rollups (expected).
       Unless you want very basic stats, only use Repaint to supplement the default stats and fill in the gaps.
       An example is to collect stats like normal every 15 minutes (or as desired) and then use Repaint once an hour.
    -  Moved the Get-FluxCredential function to the public folder (up from private) so it can be consumed by FluxorRS, an upcoming expansion module that supports Runspaces for Fluxor.
